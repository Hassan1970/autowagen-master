...
**Print PDF:** Open **`docs/CURSOR_NEW_CHAT_MASTER_PROMPT_print.html`** in Chrome/Edge → **Ctrl+P** → **Save as PDF** (or use **`docs/client_training_index.html`** → “Master prompt for new chat”).

**Full greenfield / another AI:** Use **`docs/BUILD_WHOLE_PROJECT_AI_PROMPT.md`** — detailed stage-by-stage specification to build the entire project (not the short handoff block below).
