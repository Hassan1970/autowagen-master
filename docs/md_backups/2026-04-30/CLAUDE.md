...
- **Stage 4 — Inventory & parts (+ 4b TPP, 4c batch purchases, 4d AP):** ✅
  **DONE & TESTED** *(core list/edit verified 2026-04-27 10:30 UTC+2; 4b/4c/4d
  ...
  - ✅ **Stage 4d (accounts payable — supplier & private)** — shipped *(2026-04-27)*:
    ...
    - `includes/header.php` — Inventory → **Accounts payable (owed)**.
  ...
  - ✅ **4d / supplier purchase flow re-checked (2026-04-28 UTC+2):** hassan
    confirmed **All parts** shows **Purchase #** links; **Inventory → Accounts
...
- **Stage 5 — POS:** 🟡 **CODE SHIPPED + key UX tested** *(2026-04-28 UTC+2 — hassan said **go**;
  ...
  - Pages: `invoices_admin.php` (list), `invoice_edit.php` (draft → **Finalize**, SHGA guard,
    stock/sold on parts, payments = customer **AR** balance). Nav: **POS** dropdown.
...
- **Stage 6 — Reports / shop / polish:** 🟡 **IN PROGRESS (2026-04-28 UTC+2)** — **6a / AR / statements** shipped (code on disk):
  ...
  - **`customer_ar_report.php`** — **POS → Accounts receivable (owed)** (visible to all logged-in roles). Balances = final invoices − payments; **Overdue** vs **As at**; **Overdue only** filter; **Print / PDF**; note: **not** supplier AP (that is **Inventory → Accounts payable**).
  ...
  - **Stage 6b — Public web shop (2026-04-28 code; `06b` SQL ✅ hassan 2026-04-29 UTC+2):** `parts.list_online`, `shop_orders`, `shop_order_lines`; `shop/*.php`; `shop_orders_admin.php`; **Inventory** → **Public shop (website)** / **Web shop orders**. **New + not stripped** only on web (SHGA). Payment gateway **not** in MVP — staff contact buyer after order.
  ...
  - **Stage 6e — Public shop guest enquiries (2026-04-29 code, recovered from deleted chat):**
    ...
    `shop_enquiries_admin.php` (staff inbox), helper
...
│  ├─ complete_system_manual.html          ← **full** manual: all modules, start→finish + ~30 screenshot slots (`full-NN-…`)
...
├─ customer_ar_report.php        ← Stage 6 — AR: who owes us (balances by customer)
├─ customer_statement.php         ← Stage 6 — printable customer statement; WhatsApp + mailto summary
├─ invoices_admin.php           ← Stage 5 — sales invoice list
...
│  ├─ shop_helpers.php          ← Stage 6b web shop + stock sync helpers
│  ├─ header.php
│  └─ footer.php
...
Customer balance for an invoice = `total_inc_vat` − sum(active payments).
...
Stage 3c (after `sql/03c_proof_of_residence.sql` is run): 2 more
`proof_of_residence_path`. Brings legal-papers slots to 4/4.
...
- **2026-04-28 UTC+2** — **Stage 6a (AR + account customers) coded:** `sql/06a_customer_account.sql`;
  ...
  `invoice_edit.php` (`apply_customer`, finalize vs due date); **POS → Accounts receivable (owed)**.
...
| What | How to open |
|------|-------------|
| **New sale / invoices** | **POS** → **New sale** or **Sales invoices** → `invoice_edit.php` / `invoices_admin.php` |
...
| **Who owes us (AR)** | **POS** → **Accounts receivable (owed)** → `customer_ar_report.php` (needs **`05_pos.sql`**; optional **`06a`** for “account only” filter + `invoice_edit` due-date rule). |
| **Customer account statement** | `customer_statement.php?id=N` — **Statement** link on AR report / next to name on **Customers** list. **Print / PDF**; **WhatsApp** (text summary); **Email** (`mailto:`). Attach PDF from Print for full copy. |
...
| **Who you owe (report)** | **Inventory** → **Accounts payable (owed)** → `supplier_ap_report.php` |
...
| **Public spares shop** | **`/shop/`** (e.g. `http://localhost/autowagen-master/shop/`) — no login; **New + non-stripped** parts with **List on website**. Staff: **Inventory** → **Web shop orders** → `shop_orders_admin.php`. |
...
29. Click **Inventory** → **Web shop orders**.
