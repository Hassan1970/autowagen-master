...
**Login shows “Too many failed attempts” too soon?** The app allows **six** wrong passwords per **username + IP** in **15 minutes** (see `auth/login.php`). To reset while testing: phpMyAdmin → database **`autowagen_master`** → **SQL** → run **`DELETE FROM user_login_attempts;`** → **Go**. Staff handout: **`docs/add_users_staff_guide_print.html`** section **7**.
