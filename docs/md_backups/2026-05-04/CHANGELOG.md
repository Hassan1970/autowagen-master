...
## 2026-05-04 (time unknown) UTC+2 — Master Cursor prompt file (full new-chat brain-dump)

- **Recorder:** Hassan (with Cursor assistant)
- **Summary:** **`docs/CURSOR_NEW_CHAT_MASTER_PROMPT.md`** — single place to copy/paste full project rules + CLAUDE-first + rollout Phases + stack/secrets + Reports URLs + Phase C deferral pointer; **`HOW_TO_START_NEW_CHAT.md`** intro links here; **`CLAUDE.md`** §3 docs tree + §10 quick-docs row **+ §9**.
- **Files:** `docs/CURSOR_NEW_CHAT_MASTER_PROMPT.md`, `HOW_TO_START_NEW_CHAT.md`, `CLAUDE.md`, `docs/SESSION_2026-05-04_HANDOFF.md`

## 2026-05-04 (time unknown) UTC+2 — Markdown snapshot + session handoff file

- **Recorder:** Hassan (with Cursor assistant)
- **Summary:** **`docs/md_backups/2026-05-04/`** — copies of **`CLAUDE.md`**, **`ROADMAP.md`**, **`HOW_TO_START_NEW_CHAT.md`**, **`TRAINING_SCREENSHOTS.md`**, **`CHANGELOG.md`** + **`SESSION_2026-05-04_HANDOFF.md`** (+ refreshed **`README.md`** / **`BACKLOG_POST_STAGE7.md`** in folder). **`docs/SESSION_2026-05-04_HANDOFF.md`** master diary (**Phase C** waits for client). **`HOW_TO`** after Step 3 points to session file.
- **Files:** `docs/md_backups/README.md`, `docs/md_backups/2026-05-04/*`, `HOW_TO_START_NEW_CHAT.md`, `CLAUDE.md` §9, `docs/BACKLOG_POST_STAGE7.md` opener

## 2026-05-04 (time unknown) UTC+2 — AR report staff PDF: zones A–F + illustration

- **Recorder:** Hassan (with Cursor assistant)
- **Summary:** **`docs/customer_ar_report_explained_abcdef_print.html`** — labelled **A**–**F** (title, help boxes, filters, summary cards, by customer, by invoice) plus dashed **sample** figures; **`docs/client_training_index.html`** card · **`reports_staff_guide_print.html`** §2 link · **`CLAUDE.md`** §3.
- **Files:** listed above

## 2026-05-04 (time unknown) UTC+2 — Accounts payable guide (A–D) PDF + toolbar link

- **Recorder:** Hassan (with Cursor assistant)
- **Summary:** **`docs/supplier_ap_report_explained_print.html`** — printable AP report meaning (sections A–D), payable vs receivable, deploy note so **Print / PDF** appears on live. **`supplier_ap_report.php`** — **What A–D means** button (opens handout). **`docs/client_training_index.html`** + **`reports_staff_guide_print.html`** (§1) updated.
- **Files:** above

## 2026-05-04 (time unknown) UTC+2 — Accounts payable report: Print / PDF + reader guide (A–D)

- **Recorder:** Hassan (with Cursor assistant)
- **Summary:** **`supplier_ap_report.php`** had no print path; added **Print / PDF** (`window.print()`), `@media print` (hide nav/footer, date form, help text, **Open** buttons), document title block + print tip. Screen-only **How to read this report** explains **A** top total/date, **B** table columns, **C** status/due, **D** supplier summary blocks.
- **Files:** `supplier_ap_report.php`

## 2026-05-02 (time unknown) UTC+2 — Printable handout: live DB password + secrets rotation

- **Recorder:** Hassan (with Cursor assistant)
- **Summary:** New browser handout **`docs/live_db_password_rotation_secrets_print.html`** — cPanel/MySQL password change, **`config/secrets.live.php`** on server only (`db.pass`), no secrets in chat/Git, GitHub history link if secrets were committed. Indexed in **`docs/client_training_index.html`**; **`CLAUDE.md`** §3 + quick table; **`docs/TRAINING_SCREENSHOTS.md`** table row.
- **Files:** `docs/live_db_password_rotation_secrets_print.html`, `docs/client_training_index.html`, `CLAUDE.md`, `docs/TRAINING_SCREENSHOTS.md`

## 2026-05-02 (time unknown) UTC+2 — Login lockout: 6 tries per username + IP, clear on success

- **Recorder:** Hassan (with Cursor assistant)
- **Summary:** Failed attempts are counted per **same login name (lowercased) and IP** in a **15-minute** window (`LOGIN_MAX_FAILED_IN_WINDOW = 6`). After **six** failed attempts, the next try shows the lockout message until the window rolls forward. A **successful login deletes prior failed rows** for that pair so a good password is not blocked by an old streak. (**If testing:** clear `user_login_attempts` in phpMyAdmin if the lockout message appears too soon.)
- **Files:** `auth/login.php`, `docs/add_users_staff_guide_print.html` (§7)
- **Docs synced (MD):** `CLAUDE.md` §2 Stage 1 + §8, `ROADMAP.md` Stage 1, `HOW_TO_START_NEW_CHAT.md`, `docs/TRAINING_SCREENSHOTS.md`. **`docs/add_users_staff_guide_print.html`** §7 text aligned with `auth/login.php`.

## 2026-05-02 (time unknown) UTC+2 — POS marketing brochure: screenshots wired (A–L)

- **Recorder:** Hassan (with Cursor assistant)
- **Summary:** Your captured screens are copied under stable names (**`marketing-pos-a-*.png`** … **`marketing-pos-l-*.png`**) so **`docs/pos_invoice_marketing_walkthrough_print.html`** shows real figures (open in Laragon browser → Print → PDF). **`docs/manual_screenshots/MARKETING_POS_SOURCES.md`** maps each file to the original Cursor capture id.
- **Files:** `docs/pos_invoice_marketing_walkthrough_print.html`, `docs/manual_screenshots/marketing-pos-*.png`, `docs/manual_screenshots/MARKETING_POS_SOURCES.md`

## 2026-05-02 (time unknown) UTC+2 — Customer modal: Save / Cancel reliability

- **Recorder:** Hassan (with Cursor assistant)
- **Summary:** The add/edit modal is now a **single form** spanning header, body, and footer so **Save** is a real **`type="submit"`** button (no programmatic click on a hidden submit — some browsers silently ignore that). **Cancel / Close** uses **`data-bs-dismiss="modal"`** so it only closes the dialog and cannot POST. From POS return flow, optional **Refresh list only** link keeps the **`return`** query for a full reload.
- **Files:** `customers_admin.php`

## 2026-05-02 (time unknown) UTC+2 — POS → Customers “Back to invoice” (SHGA flow)

- **Recorder:** Hassan (with Cursor assistant)
- **Summary:** From a draft invoice, **Open customers** now passes a safe **`return`** token. The customers screen shows **Back to invoice**, keeps **`return`** through save/search/pagination/edit, adds **Back to invoice** in the customer modal footer, and stops forcing the modal to stay open after save when **`return`** is present.
- **Files:** `customers_admin.php`, `invoice_edit.php`
