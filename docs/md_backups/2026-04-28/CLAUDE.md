...
- **Stage 6 — Reports / shop / polish:** 🟡 **IN PROGRESS (2026-04-28 UTC+2)** — **6a / AR / statements** shipped (code on disk):
  - ⛔ Run **`sql/06a_customer_account.sql`** on `autowagen_master` for `customers.account_customer` + `credit_limit_zar` (idempotent).
  ...
  - **Remaining Stage 6:** optional shop, more reports, SMTP email from server (not built), polish.
...
│                                 soft SKU/source prefix warning (JS)
...
columns on `vehicles` — `has_proof_of_residence` and
